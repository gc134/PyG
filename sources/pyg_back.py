...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def on_export_page2_clicked(self):
        ...
        Open a dialog to save a file, once export_page2 pressed.
    ...
    def on_calcul_page4_clicked(self):
        ...
        try :
            ...
            self.error_gc(
                "Unable to proceed with fitting.\n\nClean germination percentages data are "
                "required.\n\nTry to alter the initial parameters.\n\nTry to complete your "
                "current dataset with additional time points.")
    ...
    def on_param_ind_page4_clicked(self):
        ...
        try :
            ...
            self.error_gc(
                "Unable to produce the individual germination parameters table.\n\nPlease "
                "proceed with fitting first.")
            ...
            try :
                ...
                for i in range(0, self.df_percent_final.shape[0]):
                    ...
                    self.lag = round(np.power(((-self.df_fittedParameters.iloc[i,3] \
                                               ...
                                               /(self.df_fittedParameters.iloc[i,0]+\
                                                 self.df_fittedParameters.iloc[i,3])), \
                ...
                # creation of a second dataframe self.df_germ_parameters_transfo
                # where the Gmax values are arcsine transformed (El Kassaby et al., 2008)
                ...
                self.error_gc(
                    "Unable to produce the individual germination parameters table.\n\nSome "
                    "germination profiles may be too particular to extrapolate germination parameters.")
    ...
    def on_param_groupe_page4_clicked(self):
        ...
        try :
            ...
            self.error_gc(
                "Unable to produce the grouped germination parameters table.\n\nPlease calculate "
                "the individual parameters first.")
           ...
           self.error_gc("Please select a germination parameter in the drop-down list.")
            ...
            with sns.axes_style("whitegrid"):
                ...
                try :
                    ...
                    self.error_gc(
                        "Boxplot failed.\n\nPlease check at previous page that individual "
                        "germination parameters have been calculated.\n\nData may be insufficient "
                        "to draw boxplots for all the groups.")
        ...
        if self.comboBox_page6.currentIndex() == 0 :
           self.error_gc("Please select a germination parameter in the drop-down list.")
            ...
            try :
                ...
                self.aov = pg.anova(data=self.df_germ_parameters_transfo, dv=self.selected_param,\
                                    between='group', detailed=True).round(6)

            except :
                self.error_gc(
                    "Unable to proceed with an Anova\n\nPlease check at fitting page that "
                    "individual germination parameters have been calculated\n\nData may be "
                    "insufficient in some groups for the Anova to proceed.")
        ...
        if self.comboBox_page6.currentIndex() == 0 :
           self.error_gc("Please select a germination parameter in the drop-down list.")
            ...
            try :
                ...
                self.posthoc = pg.pairwise_ttests(data=self.df_germ_parameters_transfo,
                                                  dv=self.selected_param, between='group',
                                                  parametric=True, padjust='fdr_bh',
                                                  effsize='hedges').round(6)
            except :
                self.error_gc(
                    "Unable to proceed with multiple comparisons.\n\nPlease check at fitting page "
                    "that individual germination parameters have been correctly "
    ...
    def on_summary_page6_clicked(self) :
        ...
        try :
            ...
            self.error_gc(
                "Unable to provide the statistical summary.\n\nPlease check at fitting page that "
                "individual germination parameters have been calculated.\n\nData may be "
                "insufficient in some groups for the tests to proceed.")
    ...
    def on_pushButton_page7_clicked(self):
        ...
        """
        # plt.clf() to remove any previous matplotlib residual configuration
        plt.clf()
           ...
           self.error_gc("Please select a germination parameter in the drop-down list.")
            ...
            with sns.axes_style("whitegrid"):
                ...
                try :
                    ...
                    self.error_gc(
                        "Tukey's boxplot failed.\n\nPlease check at fitting page that individual "
                        "germination parameters have been calculated.\n\nData may be insufficient "
                        "in some groups to draw boxplots and to perform multiple comparisons.")
